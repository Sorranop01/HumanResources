/**
 * Cloud Function: Approve Leave Request
 * ✅ Layer 3 - Zod Validation at Cloud Function Level
 */

import { FieldValue, getFirestore } from 'firebase-admin/firestore';
import { logger } from 'firebase-functions/v2';
import { HttpsError, onCall } from 'firebase-functions/v2/https';
import { CloudFunctionApproveLeaveRequestSchema } from '@/domains/people/features/leave/schemas/index.js';

/**
 * Approve Leave Request
 * POST /approveLeaveRequest
 */
export const approveLeaveRequest = onCall(
  {
    region: 'asia-southeast1',
    timeoutSeconds: 60,
    cors: true,
  },
  async (request) => {
    const db = getFirestore();
    const { auth, data } = request;

    // ===== 1. Authentication Check =====
    if (!auth) {
      logger.warn('Unauthenticated request to approveLeaveRequest');
      throw new HttpsError('unauthenticated', 'ต้องเข้าสู่ระบบก่อนใช้งาน');
    }

    const userId = auth.uid;
    logger.info(`Approving leave request by user: ${userId}`);

    // ===== 2. Input Validation with Zod =====
    const validation = CloudFunctionApproveLeaveRequestSchema.safeParse(data);

    if (!validation.success) {
      const errorMessages = validation.error.issues
        .map((issue) => `${issue.path.join('.')}: ${issue.message}`)
        .join(', ');

      logger.error('Zod validation failed', {
        errors: validation.error.issues,
        errorCount: validation.error.issues.length,
      });

      throw new HttpsError(
        'invalid-argument',
        `การตรวจสอบข้อมูลล้มเหลว: ${errorMessages}`,
        validation.error.issues
      );
    }

    // Use validated data (100% type-safe)
    const { requestId, approverId, comments } = validation.data;

    try {
      // ===== 3. Fetch Leave Request =====
      const leaveRequestRef = db.collection('leaveRequests').doc(requestId);
      const leaveRequestDoc = await leaveRequestRef.get();

      if (!leaveRequestDoc.exists) {
        throw new HttpsError('not-found', `ไม่พบคำขอลา: ${requestId}`);
      }

      const leaveRequestData = leaveRequestDoc.data();
      if (!leaveRequestData) {
        throw new HttpsError('internal', 'ข้อมูลคำขอลาไม่ถูกต้อง');
      }

      // ===== 4. Verify Request Status =====
      if (leaveRequestData.status !== 'pending') {
        throw new HttpsError(
          'failed-precondition',
          `ไม่สามารถอนุมัติคำขอลาที่มีสถานะ: ${leaveRequestData.status}`
        );
      }

      // ===== 5. Verify Approver Permission =====
      const approvalChain = leaveRequestData.approvalChain || [];
      const currentLevel = leaveRequestData.currentApprovalLevel || 1;

      const currentStep = approvalChain.find((step: any) => step.level === currentLevel);

      if (!currentStep) {
        throw new HttpsError('internal', 'ไม่พบขั้นตอนการอนุมัติในระบบ');
      }

      if (currentStep.approverId !== approverId) {
        logger.warn(`User ${userId} attempted to approve request as ${approverId}`, {
          requestId,
          expectedApproverId: currentStep.approverId,
        });
        throw new HttpsError('permission-denied', 'คุณไม่มีสิทธิ์อนุมัติคำขอลานี้');
      }

      // Verify user is linked to this employee
      const userRef = db.collection('users').doc(userId);
      const userDoc = await userRef.get();
      const userData = userDoc.data();
      const userEmployeeId = userData?.employeeId;

      if (userEmployeeId !== approverId) {
        logger.warn(`User ${userId} does not match approverId ${approverId}`, {
          userEmployeeId,
          requestId,
        });
        throw new HttpsError('permission-denied', 'คุณไม่มีสิทธิ์อนุมัติคำขอลานี้');
      }

      // ===== 6. Update Approval Chain =====
      const now = FieldValue.serverTimestamp();

      // Update current step
      const updatedApprovalChain = approvalChain.map((step: any) => {
        if (step.level === currentLevel) {
          return {
            ...step,
            status: 'approved',
            actionAt: now,
            comments: comments || null,
          };
        }
        return step;
      });

      // Determine if all approvals are complete
      const hasMoreApprovals = updatedApprovalChain.some(
        (step: any) => step.level > currentLevel && step.status === 'pending'
      );

      const finalStatus = hasMoreApprovals ? 'pending' : 'approved';
      const nextLevel = hasMoreApprovals ? currentLevel + 1 : currentLevel;

      // ===== 7. Update Leave Request =====
      await leaveRequestRef.update({
        approvalChain: updatedApprovalChain,
        currentApprovalLevel: nextLevel,
        status: finalStatus,
        updatedAt: now,
        updatedBy: userId,
      });

      // ===== 8. Update Leave Entitlement (if fully approved) =====
      if (finalStatus === 'approved') {
        const leaveTypeRef = db.collection('leaveTypes').doc(leaveRequestData.leaveTypeId);
        const leaveTypeDoc = await leaveTypeRef.get();
        const leaveTypeData = leaveTypeDoc.data();

        if (leaveTypeData?.requiresEntitlement) {
          const entitlementSnapshot = await db
            .collection('leaveEntitlements')
            .where('employeeId', '==', leaveRequestData.employeeId)
            .where('leaveTypeId', '==', leaveRequestData.leaveTypeId)
            .limit(1)
            .get();

          if (!entitlementSnapshot.empty) {
            const entitlementDoc = entitlementSnapshot.docs[0];
            if (!entitlementDoc) {
              throw new HttpsError('not-found', 'ไม่พบข้อมูลสิทธิ์การลา');
            }
            const entitlementData = entitlementDoc.data();
            const remainingDays = entitlementData?.remainingDays || 0;
            const totalDays = leaveRequestData.totalDays || 0;

            await entitlementDoc.ref.update({
              remainingDays: remainingDays - totalDays,
              usedDays: (entitlementData?.usedDays || 0) + totalDays,
              updatedAt: now,
            });
          }
        }
      }

      logger.info(`Leave request approved successfully: ${requestId}`, {
        requestNumber: leaveRequestData.requestNumber,
        approverId,
        currentLevel,
        finalStatus,
      });

      return {
        success: true,
        message: finalStatus === 'approved' ? 'อนุมัติคำขอลาสำเร็จ' : 'อนุมัติขั้นตอนปัจจุบันสำเร็จ',
        data: {
          id: requestId,
          requestNumber: leaveRequestData.requestNumber,
          status: finalStatus,
          currentLevel: nextLevel,
        },
      };
    } catch (error) {
      // Re-throw HttpsError
      if (error instanceof HttpsError) {
        throw error;
      }

      logger.error('Failed to approve leave request', {
        error,
        requestId,
        approverId,
      });

      throw new HttpsError('internal', 'เกิดข้อผิดพลาดในการอนุมัติคำขอลา');
    }
  }
);
