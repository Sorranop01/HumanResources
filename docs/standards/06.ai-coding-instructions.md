...
description: Master coding rules for React + Vite + TypeScript Monorepo — Human Resources System
refs:
...
# 🧠 06 — AI Coding Instructions (Human Master Guide)

**Purpose:**  
This document defines all coding principles, rules, and constraints that AI must follow when writing or modifying code in the **Human Resources Monorepo** — a React + Firebase-based HR System for managing employees, teams, candidates, and performance.

---

## 🇬🇧 Section 1: AI Master Coding Rules
...
⚠️ No deviation or personal style is allowed unless explicitly requested.
...
- Avoid using enums for strings — use union types instead.
...
✅ Follow TypeScript + Biome + FSD standards  
✅ Contain only code (no inline explanations)

---

### 14. Firestore Data Modeling — must follow `@/standards/07`
...
- Use UTC timestamps.
- Multi-document writes → use Cloud Functions (transactions).

---

### 15. Firebase Functions (ESM v2) — must follow `@/standards/08`
...
Human HR Monorepo follows **three golden principles:**

1. **Type Safety** — every layer must use strict TypeScript types.
2. **Domain Isolation** — code lives inside its domain/feature only.
3. **Consistency** — always align with Biome, FSD, and Firestore data rules.

---

> 🧩 Part of the official **Human Developer Handbook**  
